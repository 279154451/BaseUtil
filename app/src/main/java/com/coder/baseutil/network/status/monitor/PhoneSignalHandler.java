...
import com.coder.baseutil.network.status.utils.PermissionUtil;
import com.coder.baseutil.network.status.utils.PhoneUtil;
import com.coder.baseutil.network.status.utils.ReflectUtil;
...
public class PhoneSignalHandler {
    ...
    private PhoneSignalHandler(Context context) {
        mContext = context;
    ...
    public static PhoneSignalHandler getInstance(Context context) {
        if (null == mInstance) {
            synchronized (mLock) {
                if (null == mInstance) {
                    mInstance = new PhoneSignalHandler(context);
                }
            }
        }
        return mInstance;
