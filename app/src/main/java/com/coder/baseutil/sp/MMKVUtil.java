...
public class MMKVUtil {
     ...
     * @return 保存结果
     */
    public boolean putObject(String key, Object object, TypeToken typeToken) {
        boolean result;
        try {
            String json = GsonUtil.object2Json(object, typeToken.getType());
            LogUtil.getUtil().e("MMKV", "putObject: " + json);
            put(key, json);
            result = true;
        } catch (Exception e) {
            result = false;
     ...
     * @return
     */
    public <V> V getObject(String key, TypeToken typeToken) {

        if (mmkv != null) {
            String json = (String) get(key,"");
            LogUtil.getUtil().e("MMKV", "getObject: " + json);
